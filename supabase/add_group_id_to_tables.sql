...
-- UPDATED: Only adds to tables that exist in KinPulse

-- IMPORTANT: Run this AFTER create_groups_schema.sql

-- 1. Add group_id to messages table (if exists)
DO $$
BEGIN
...
-- 10. Add group_id to photos table (if exists - for future use)
DO $$
BEGIN
...
-- 11. Add group_id to calendar_events table (if exists)
DO $$
BEGIN
...
-- 12. Add group_id to reactions table (if exists)
DO $$
BEGIN
